...
bool ATCRadioStack::_process_radio(const std::map<void *, audio::SampleType[audio::frameSizeSamples]> &sampleCache, std::map<void *, audio::SampleType[audio::frameSizeSamples]> &eqSampleCache, size_t rxIter, std::shared_ptr<OutputDeviceState> state) {
    ...
    // We only allow split channels on the headset, not much point on the speakers
    if (mRadioState[rxIter].onHeadset) {
        ...
        // We don't have split channels, we throw everything in the same buffer
...
audio::SourceStatus ATCRadioStack::getAudioFrame(audio::SampleType *bufferOut, bool onHeadset) {
    ...
    if (onHeadset) {
        ...
        _interleave(state->mLeftMixingBuffer, state->mRightMixingBuffer, interleavedSamples, audio::frameSizeSamples);
    ...
    for (auto &state: mRadioState) {
        if (state.second.transceivers.empty()) {
            ...
            for (auto &trans: state.second.transceivers) {
                retSet.emplace_back(i, trans.Frequency, trans.LatDeg, trans.LonDeg, trans.HeightMslM,
                                    trans.HeightAglM);
                trans.ID = i;
                i++;
            }
...
void ATCRadioStack::addFrequency(unsigned int freq, bool onHeadset, std::string stationName, HardwareType hardware, PlaybackChannel channel) {
    std::lock_guard<std::mutex> mRadioStateGuard(mRadioStateLock);
