...
OutputAudioDevice::OutputAudioDevice(std::weak_ptr<ATCRadioStack> stack,
                                     bool isHeadset)
    : mStack(stack), isHeadset(isHeadset) {}

audio::SourceStatus
OutputAudioDevice::getAudioFrame(audio::SampleType *bufferOut) {
...
ATCRadioStack::ATCRadioStack(struct event_base *evBase,
    ...
    : mEvBase(evBase), mChannel(), mPtt(false), mAtisRecord(false),
      mAtisPlayback(false), mRT(false), IncomingAudioStreams(0),
      mResources(std::move(resources)), mStreamMapLock(), cacheNum(0),
      mIncomingStreams(), mLastFramePtt(false), mRadioStateLock(),
      mTxSequence(0), mRadioState(),
      mVoiceSink(std::make_shared<VoiceCompressionSink>(*this)),
      mMaintenanceTimer(
          mEvBase, std::bind(&ATCRadioStack::maintainIncomingStreams, this)),
...
void ATCRadioStack::setupDevices(
    util::ChainedCallback<void(ClientEventType, void *, void *)>
        *eventCallback) {
  mHeadsetDevice =
      std::make_shared<OutputAudioDevice>(shared_from_this(), true);
  mSpeakerDevice =
      std::make_shared<OutputAudioDevice>(shared_from_this(), false);
...
void ATCRadioStack::mix_buffers(audio::SampleType *RESTRICT src_dst,
                                const audio::SampleType *RESTRICT src2,
                                float src2_gain) {
...
bool ATCRadioStack::mix_effect(std::shared_ptr<audio::ISampleSource> effect,
                               float gain,
                               std::shared_ptr<OutputDeviceState> state) {
  if (effect && gain > 0.0f) {
    auto rv = effect->getAudioFrame(state->mFetchBuffer);
    if (rv == audio::SourceStatus::OK) {
      ATCRadioStack::mix_buffers(state->mChannelBuffer, state->mFetchBuffer,
                                 gain);
    } else {
      return false;
    }
...
void ATCRadioStack::set_radio_effects(size_t rxIter, float crackleGain,
                                      float &whiteNoiseGain) {
  whiteNoiseGain = fxWhiteNoiseGain;
  if (whiteNoiseGain > 0.0f) {
    if (!mRadioState[rxIter].WhiteNoise) {
      mRadioState[rxIter].WhiteNoise =
          std::make_shared<audio::PinkNoiseGenerator>();
    }
  }
  if (crackleGain > 0.0f) {
    if (!mRadioState[rxIter].Crackle) {
      mRadioState[rxIter].Crackle =
          std::make_shared<audio::RecordedSampleSource>(mResources->mCrackle,
                                                        true);
    }
  }
}
...
bool ATCRadioStack::_process_radio(
    const std::map<void *, audio::SampleType[audio::frameSizeSamples]>
        &sampleCache,
  ...
  for (auto &srcPair : mIncomingStreams) {
    ...
    std::copy_if(srcPair.second.transceivers.begin(),
                 srcPair.second.transceivers.end(),
                 std::back_inserter(matchingTransceivers),
                 [&](afv::dto::RxTransceiver k) {
    ...
    if (matchingTransceivers.size() > 0) {
      ...
      auto closestTransceiver = *std::max_element(
          matchingTransceivers.begin(), matchingTransceivers.end(),
          [](afv::dto::RxTransceiver a, afv::dto::RxTransceiver b) {
      ...
      if (!mRadioState[rxIter].mBypassEffects) {
        ...
        crackleFactor = static_cast<float>(
            (exp(closestTransceiver.DistanceRatio) *
  ...
  if (concurrentStreams > 0) {
    ...
    if (concurrentStreams > 1) {
      if (!mRadioState[rxIter].BlockTone) {
        mRadioState[rxIter].BlockTone =
            std::make_shared<audio::SineToneSource>(fxBlockToneFreq);
      }
  ...
  if (!mix_effect(mRadioState[rxIter].Click,
...
audio::SourceStatus ATCRadioStack::getAudioFrame(audio::SampleType *bufferOut,
                                                 bool onHeadset) {

  std::shared_ptr<OutputDeviceState> state =
      onHeadset ? mHeadsetState : mSpeakerState;
  ...
  ::memcpy(bufferOut, state->mMixingBuffer,
           sizeof(audio::SampleType) * audio::frameSizeSamples);
  return audio::SourceStatus::OK;
}

bool ATCRadioStack::_packetListening(
    const afv::dto::AudioRxOnTransceivers &pkt) {
    ...
    mChannel->registerDtoHandler(
        "AR", [this](const unsigned char *data, size_t len) {
          try {
            ...
            auto objHdl =
                msgpack::unpack(reinterpret_cast<const char *>(data), len);
            objHdl.get().convert(rxAudio);
            this->rxVoicePacket(rxAudio);
          } catch (const msgpack::type_error &e) {
            LOG("atcradiostack", "unable to unpack audio data received: %s",
                e.what());
...
void ATCRadioStack::setClientPosition(double lat, double lon, double amslm,
                                      double aglm) {
  mClientLatitude = lat;
...
void ATCRadioStack::setTransceivers(
    unsigned int freq, std::vector<afv::dto::StationTransceiver> transceivers) {
  ...
  for (auto inTrans : transceivers) {
    ...
    dto::Transceiver out(0, freq, inTrans.LatDeg, inTrans.LonDeg,
...
std::vector<afv::dto::CrossCoupleGroup>
ATCRadioStack::makeCrossCoupleGroupDto() {
...
/** Audio enters here from the Codec Compressor before being sent out on to the
 * network.
 *
 *
 *
 *
 */
void ATCRadioStack::processCompressedFrame(
...
void ATCRadioStack::setPtt(bool pressed) { mPtt.store(pressed); }
...
bool ATCRadioStack::getAtisRecording() { return mAtisRecord.load(); }
...
bool ATCRadioStack::isAtisPlayingBack() { return mAtisPlayback.load(); }
...
void ATCRadioStack::setRT(bool active) { mRT.store(active); }
...
void ATCRadioStack::maintainIncomingStreams() {
  ...
  for (const auto &streamPair : mIncomingStreams) {
    ...
    if ((now - streamPair.second.source->getLastActivityTime()) >
        audio::compressedSourceCacheTimeoutMs) {
