...
ATCClient::ATCClient(struct event_base *evBase,
                     const std::string &resourceBasePath,
                     const std::string &clientName, std::string baseUrl)
    : mFxRes(std::make_shared<afv::EffectResources>(resourceBasePath)),
      mEvBase(evBase), mTransferManager(mEvBase),
      ...
      mATCRadioStack(std::make_shared<afv::ATCRadioStack>(
          mEvBase, mFxRes, &mVoiceSession.getUDPChannel())),
      mAudioDevice(), mSpeakerDevice(),
      //        mClientLatitude(0.0),
      //        mClientLongitude(0.0),
      //        mClientAltitudeMSLM(100.0),
      //        mClientAltitudeGLM(100.0),
      mCallsign(), mTxUpdatePending(false), mWantPtt(false), mPtt(false),
      mAtisRecording(false),
      mTransceiverUpdateTimer(
          mEvBase, std::bind(&ATCClient::sendTransceiverUpdate, this)),
      mClientName(clientName), mAudioApi(0), mAudioInputDeviceName(),
      mAudioOutputDeviceName(), ClientEventCallback() {
  mAPISession.StateCallback.addCallback(
      this,
      std::bind(&ATCClient::sessionStateCallback, this, std::placeholders::_1));
  mAPISession.AliasUpdateCallback.addCallback(
      this, std::bind(&ATCClient::aliasUpdateCallback, this));
  mAPISession.StationTransceiversUpdateCallback.addCallback(
      this, std::bind(&ATCClient::stationTransceiversUpdateCallback, this,
                      std::placeholders::_1));
  mAPISession.StationVccsCallback.addCallback(
      this, std::bind(&ATCClient::stationVccsCallback, this,
                      std::placeholders::_1, std::placeholders::_2));
  mAPISession.StationSearchCallback.addCallback(
      this, std::bind(&ATCClient::stationSearchCallback, this,
                      std::placeholders::_1, std::placeholders::_2));
  mVoiceSession.StateCallback.addCallback(
      this,
      std::bind(&ATCClient::voiceStateCallback, this, std::placeholders::_1));
...
void ATCClient::setClientPosition(double lat, double lon, double amslm,
                                  double aglm) {
...
void ATCClient::setCredentials(const std::string &username,
...
void ATCClient::voiceStateCallback(afv::VoiceSessionState state) {
  ...
  case afv::VoiceSessionState::Connected:
    ...
    startAudio();
    queueTransceiverUpdate();
    ClientEventCallback.invokeAll(ClientEventType::VoiceServerConnected,
                                  nullptr, nullptr);
    break;
  case afv::VoiceSessionState::Disconnected:
    ...
    ClientEventCallback.invokeAll(ClientEventType::VoiceServerDisconnected,
                                  nullptr, nullptr);
    break;
  case afv::VoiceSessionState::Error:
    ...
    if (voiceError == afv::VoiceSessionError::UDPChannelError) {
      ...
      ClientEventCallback.invokeAll(ClientEventType::VoiceServerChannelError,
                                    &channelErrno, nullptr);
    } else {
      ClientEventCallback.invokeAll(ClientEventType::VoiceServerError,
                                    &voiceError, nullptr);
    }
    break;
  }
...
void ATCClient::sessionStateCallback(afv::APISessionState state) {
  ...
  case afv::APISessionState::Disconnected:
    LOG("afv_native::ATCClient",
        "Disconnected from AFV API Server.  Terminating sessions");
    // because we only ever commence a normal API Session teardown from a
    // voicesession hook, we don't need to call into voiceSession in this case
    // only.
    ClientEventCallback.invokeAll(ClientEventType::APIServerDisconnected,
                                  nullptr, nullptr);
    break;
  case afv::APISessionState::Error:
    LOG("afv_native::ATCClient",
        "Got error from AFV API Server.  Disconnecting session");
    sessionError = mAPISession.getLastError();
    ClientEventCallback.invokeAll(ClientEventType::APIServerError,
                                  &sessionError, nullptr);
    break;
  default:
...
void ATCClient::startAudio() {
  ...
  if (!mSpeakerDevice) {
    ...
    mSpeakerDevice =
        audio::AudioDevice::makeDevice("afv::speaker", mAudioSpeakerDeviceName,
                                       mAudioInputDeviceName, mAudioApi);
    ...
    if (!mSpeakerDevice) {
      ...
      mSpeakerDevice->setNotificationFunc(
          std::bind(&ATCClient::deviceStoppedCallback, this,
                    std::placeholders::_1, std::placeholders::_2));
    }
  } else {
  ...
  if (!mAudioDevice) {
    ...
    mAudioDevice = audio::AudioDevice::makeDevice(
        "afv::headset", mAudioOutputDeviceName, mAudioInputDeviceName,
        mAudioApi, mHeadsetOutputChannel);
    ...
    if (!mAudioDevice) {
      ...
      mAudioDevice->setNotificationFunc(
          std::bind(&ATCClient::deviceStoppedCallback, this,
                    std::placeholders::_1, std::placeholders::_2));
    }
  } else {
...
void ATCClient::sendTransceiverUpdate() {
  ...
  mVoiceSession.postTransceiverUpdate(
      transceiverDto, [this](http::Request *r, bool success) {
  ...
  mVoiceSession.postCrossCoupleGroupUpdate(
      mATCRadioStack->makeCrossCoupleGroupDto(),
      [this](http::Request *r, bool success) {
        if (!success) {
          LOG("ATCClient",
              "Failed to post cross couple transceivers update with code %s",
...
bool ATCClient::isAtisRecording() { return mAtisRecording; }
...
void ATCClient::stopAtisPlayback() { mATCRadioStack->stopAtisPlayback(); };
...
bool ATCClient::isAtisListening() { return mATCRadioStack->isAtisListening(); };

void ATCClient::setPtt(bool pttState) {
  if (pttState) {
    mWantPtt = true;
    // if we're setting the Ptt, we have to check a few things.
    // if we're still pending an update, and the radios are out of step, guard
    // the Ptt.
    if (mTxUpdatePending) {
      if (!mTxUpdatePending) {
...
void ATCClient::setRT(bool rtState) { mATCRadioStack->setRT(rtState); }
...
bool ATCClient::isAPIConnected() const {
  ...
  return sState == afv::APISessionState::Running ||
         sState == afv::APISessionState::Reconnecting;
}

bool ATCClient::isVoiceConnected() const { return mVoiceSession.isConnected(); }

void ATCClient::setBaseUrl(std::string newUrl) {
  mAPISession.setBaseUrl(std::move(newUrl));
}

void ATCClient::stopTransceiverUpdate() { mTransceiverUpdateTimer.disable(); }

void ATCClient::setAudioApi(audio::AudioDevice::Api api) { mAudioApi = api; }
...
void ATCClient::stationVccsCallback(std::string stationName,
                                    std::map<std::string, unsigned int> vccs) {
  ClientEventCallback.invokeAll(ClientEventType::VccsReceived, &stationName,
                                &vccs);
}

void ATCClient::stationSearchCallback(
    bool found, std::pair<std::string, unsigned int> data) {
...
void ATCClient::stationTransceiversUpdateCallback(std::string stationName) {
  ...
  if (linkNewTransceiversFrequencyFlag > 0) {
    ...
    if (transceivers[stationName].size() > 0)
      ...
      LOG("ATCClient",
          "Tried to acquire new transceivers but did not find any for station");

    linkNewTransceiversFrequencyFlag = -1;
  } else {
    // We got new station transceivers that we don't need to link immediately,
    // but can wait until the next transceiver update

    auto transceivers = getStationTransceivers();
    if (transceivers[stationName].size() > 0) {
      auto it = std::find_if(mATCRadioStack->mRadioState.begin(),
                             mATCRadioStack->mRadioState.end(),
                             [&stationName](const auto &t) {
                               return t.second.stationName == stationName;
                             });
      if (it != mATCRadioStack->mRadioState.end()) {
        mATCRadioStack->setTransceivers(it->second.Frequency,
                                        transceivers[stationName]);
      }
    }
  }
...
std::map<std::string, std::vector<afv::dto::StationTransceiver>>
ATCClient::getStationTransceivers() const {
